# travis ci stopped executing build

https://www.google.com/search?q=travis+ci+stopped+executing+build&oq=travis+ci+stopped+executing+build&aqs=chrome..69i57j33.15520j0j7&sourceid=chrome&ie=UTF-8

https://blog.travis-ci.com/2014-05-12-why-is-my-build-not-running

# travis could not parse

https://www.google.com/search?q=travis+could+not+parse&oq=travis+could+not+&aqs=chrome.1.69i57j0l7.8722j0j7&sourceid=chrome&ie=UTF-8

https://stackoverflow.com/questions/50551277/could-not-parse-travis-yml

http://www.yamllint.com/

# did not find expected key while parsing a block mapping

https://www.google.com/search?q=did+not+find+expected+key+while+parsing+a+block+mapping&oq=did+not+find+expected+key+while+parsing+a+block+mapping&aqs=chrome..69i57j0l7.770j0j7&sourceid=chrome&ie=UTF-8

https://github.com/ruby/psych/issues/190

# Exit code: 1 - javadoc: error - No public or protected classes found to document.

https://www.google.com/search?q=Exit+code%3A+1+-+javadoc%3A+error+-+No+public+or+protected+classes+found+to+document.&oq=Exit+code%3A+1+-+javadoc%3A+error+-+No+public+or+protected+classes+found+to+document.&aqs=chrome..69i57.1054j0j7&sourceid=chrome&ie=UTF-8

http://maven.40175.n5.nabble.com/javadoc-plugin-claims-no-sources-but-there-s-one-td4712225.html

https://stackoverflow.com/questions/23542876/unable-to-build-maven-project-due-to-javadoc-error


